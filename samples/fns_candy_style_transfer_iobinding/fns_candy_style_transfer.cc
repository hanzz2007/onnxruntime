...
int main(int argc, char* argv[]) {
  ...
  Ort::MemoryInfo cuda_mem_info("Cuda", OrtArenaAllocator, 0, OrtMemTypeDefault);
  io_binding.BindOutput("outputImage", cuda_mem_info);
  ...
  Ort::Allocator cpu_allocator(session, mem_info);
  auto out_shape = output_tensor.GetTensorTypeAndShapeInfo().GetShape();
  auto output_cpu_tensor = Ort::Value::CreateTensor<float>(cpu_allocator, out_shape.data(), out_shape.size());
  auto n = std::accumulate(out_shape.begin(), out_shape.end(), (int64_t)1, std::multiplies<int64_t>());
  auto output_cpu_tensor_data = output_cpu_tensor.GetTensorMutableData<float>();
  cudaMemcpy(output_cpu_tensor_data, output_tensor_data, n * sizeof(float), cudaMemcpyDeviceToHost);

  output_cpu_tensor.GetTensorTypeAndShapeInfo().GetShape();
  output_cpu_tensor.GetTensorTypeAndShapeInfo().GetSymbolicDimensions();


  uint8_t* output_image_data = NULL;
  chw_to_hwc(output_cpu_tensor_data, 720, 720, &output_image_data);
